...
y_recode = zeros(size(a3, 1), m);
for i = 1:m
    y_recode(y(i), i) = 1;
end

J = - trace(y_recode' * log(a3) + (1-y_recode)' * log(1 - a3))/m;
...
reg = lambda * (trace(Theta1_hat * Theta1_hat') + trace(Theta2_hat * Theta2_hat'))/(2*m);
J = J + reg;
